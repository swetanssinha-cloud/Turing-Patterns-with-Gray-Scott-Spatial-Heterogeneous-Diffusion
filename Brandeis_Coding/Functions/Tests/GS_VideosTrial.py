...
Du, Dv = 0.16, 0.08
F, k = 0.046, 0.064 #0.046, 0.064 (Stripes) #0.028, 0.064 (Dots) #0.010, 0.046 (Dynamic) #0.037, 0.06 (Standard)
N = 200
L = 200
dx = L / N
dt = 1
n_steps =  300 #10000 #Using 300 for video. 
n_frames = 300 #for video
steps_per_frame = 100 #for video
...
for i in range(n_frames):
    ...
    plt.axis('off')
    frame_filenmae = os.path.join(frame_dir, f'frame_{i:04d}.png')
    plt.savefig(frame_filenmae, bbox_inches='tight', pad_inches=0, dpi=150)
    plt.clf()
