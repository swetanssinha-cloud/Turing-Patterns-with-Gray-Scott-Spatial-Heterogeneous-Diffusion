...
def first_derivative(M, axis):
    firstD = (np.roll(M, 1, axis) - np.roll(M, -1, axis)) / (2 * dx) #For x derviative use axis = 0 and for y use axis =1
    return firstD
...
def update(frame, U, V, Du, Dv, F, k, dx, dt,total_t):
    for _ in range(100):
...
ani = animation.FuncAnimation(fig, update, frames=n_steps, interval = 0.1, blit=False, fargs=(U, V, Du, Dv, F, k, dx, dt, total_t))
#ani.save('Gray_Scott_Animation.mp4', writer='ffmpeg', fps=30, dpi=300) #Save the animation as a video file
plt.show()

dVx = first_derivative(V, 0)
dVy = first_derivative(V, 1)
...
plt.quiver(x, y, dVx, dVy, color='blue', scale=30)
