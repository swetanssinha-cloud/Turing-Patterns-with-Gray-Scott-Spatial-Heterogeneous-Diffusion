...
Du_original, Dv_original = 0.16, 0.08
...
def solver(U, V, Du, Dv, F, k, dx, dt):
    ...
    U += (Du_original * Lu - uvv + F * (1 - U)) * dt
    V += (Dv_original * Lv + uvv - (F+k) * V) * dt
...
def update(frame, U, V, Du, Dv, F, k, dx, dt):
    for _ in range(100):
        for i in U[i]:
            if i > 80:
                Du = Du_original * 2  # Example modification for space
            else:
                Du = Du_original
        U, V = solver(U, V, Du, Dv, F, k, dx, dt)
    im.set_array(V)
    return [im]
ani = animation.FuncAnimation(fig, update, frames=n_steps, interval = 0.1, blit=True, fargs=(U, V, Du_original, Dv_original, F, k, dx, dt))
